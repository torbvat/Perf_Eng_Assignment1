...
def writeToFile_Container(containerSet):
        ...
        for stack in section:
            for containerCell in stack:
                for container in containerCell:
                    if container != 0:
                        ship.unloadContainer(container.serialNumber)
                        containersOnDock.append(container)
    return containersOnDock
    ...
    for i in range(4):
        for containerCell in ordered_containerCells[len(ordered_containerCells)//4*i:len(ordered_containerCells)//4*(i+1)]:
            if containerCell[1].length == 20:
                ...
                continue
    print("Amount of containers in the four sections:", amountOfContainersInSections)
